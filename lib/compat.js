...
function rmdir(path, options) {
  if (options && options.recursive)
    return _rmdir(fromPath(path));
  return call(fs.rmdir, [path]);
}

function rmdirSync(path, options) {
  if (options && options.recursive)
    return _rmdirSync(fromPath(path));
  return fs.rmdirSync(path);
}

async function _rmdir(file) {
  let stat = null;

  try {
    stat = await call(fs.lstat, [file]);
  } catch (e) {
    if (e.code === 'ENOENT')
      return;
    throw e;
  ...
  if (stat.isDirectory()) {
    ...
    try {
      list = await call(fs.readdir, [file]);
    } catch (e) {
      if (e.code === 'ENOENT')
        return;
      throw e;
    ...
    for (const name of list)
      await _rmdir(path.join(file, name));

    try {
      await call(fs.rmdir, [file]);
    } catch (e) {
      if (e.code === 'ENOENT')
        return;
      throw e;
...
function _rmdirSync(file) {
  ...
  if (stat.isDirectory()) {
    ...
    for (const name of list)
      _rmdirSync(path.join(file, name));
...
async function writev(fd, buffers, position) {
  if (!Array.isArray(buffers))
    throw new ArgError('buffers', buffers, 'Array');

  let written = 0;

  for (const array of buffers) {
    const buf = toBuffer(array);
    const bytes = await write(fd, buf, 0, buf.length, position);
...
function writevSync(fd, buffers, position) {
  ...
  for (const array of buffers) {
    const buf = toBuffer(array);
    const bytes = writeSync(fd, buf, 0, buf.length, position);
