...
async function empty(file, mode) {
  let list = null;

  try {
    list = await fs.readdir(file);
  } catch (e) {
    if (e.code === 'ENOENT')
      return mkdirp(file, mode);
    throw e;
  }

  for (const name of list)
    await remove(join(file, name));

  return undefined;
}

function emptySync(file, mode) {
  let list = null;
  ...
  try {
    ...
    if (e.code === 'ENOENT')
      return mkdirpSync(file, mode);
    throw e;
  }

  for (const name of list)
    removeSync(join(file, name));
