...
const ASYNC_ITERATOR = Symbol.asyncIterator || Symbol.iterator;
const HAS_ASYNC = ASYNC_ITERATOR === Symbol.asyncIterator;
...
async function traverse(root, options, cb) {
  if (Array.isArray(root)) {
    for (const file of root)
      await traverseSync(fromPath(file), options, cb);
    return;
  }
...
exports.walk = HAS_ASYNC ? walk : null;
